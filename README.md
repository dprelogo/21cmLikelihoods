...
### Mean constraint
In order to estimate the mean better, a feed-forward NN is used which takes parameters $\bm{\theta}$ and outputs the mean:
$$\bm{\mu}_{\text{NN}}(\bm{\theta}) = \text{NN}(\bm{\theta}) \, . $$
The possible Gaussian likelihoods are then:
$$\mathcal{L}_{\text{NN}}(\bm{d}_{PS} | \bm{\theta}) &= \mathcal{N}(\bm{d}| \bm{\mu}_{\text{NN}}(\bm{\theta}), \bm{\sigma}^2(\bm{\theta}_{\text{fid}})) \, ,$$
$$\mathcal{L}_{\text{NN}}(\bm{d}_{PS} | \bm{\theta}) &= \mathcal{N}(\bm{d}| \bm{\mu}_{\text{NN}}(\bm{\theta}), \Sigma(\bm{\theta}_{\text{fid}})) \, .$$
Here $\bm{\sigma}^2(\bm{\theta}_{\text{fid}})$ and $\Sigma(\bm{\theta}_{\text{fid}})$ represent the variance and covariance estimated at the fiducial parameter values.
...
$$\bm{\mu}_{\text{NN}}(\bm{\theta}), \bm{\sigma}^2_{\text{NN}}(\bm{\theta}) = \text{NN}(\bm{\theta}) \, , $$
$$\bm{\mu}_{\text{NN}}(\bm{\theta}), \Sigma_{\text{NN}}(\bm{\theta}) = \text{NN}(\bm{\theta}) \, , $$
with their respective likelihoods:
$$\mathcal{L}_{\text{NN}}(\bm{d}_{PS} | \bm{\theta}) &= \mathcal{N}(\bm{d}_{PS}| \bm{\mu}_{\text{NN}}(\bm{\theta}), \bm{\sigma}^2_{\text{NN}}(\bm{\theta})) \, ,$$
$$\mathcal{L}_{\text{NN}}(\bm{d}_{PS} | \bm{\theta}) &= \mathcal{N}(\bm{d}_{PS}| \bm{\mu}_{\text{NN}}(\bm{\theta}), \Sigma_{\text{NN}}(\bm{\theta})) \, .$$
